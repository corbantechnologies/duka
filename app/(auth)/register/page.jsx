...
const formSchema = z.object({
  first_name: z.string({required_error: "Please enter your first name.",}).min(2, {
    message: "First name must be at least 2 characters.",
  }),
  last_name: z.string({required_error: "Please enter your last name.",}).min(2, {
  ...
  phone: z.string({required_error: "Please enter your phone number.",}).min(4),
  ...
  const handleFileChange = (e) => {
    ...
    const form = useForm({
      ...
      async function onSubmit(values) {
        setLoading(true);
        if (values.accountType === 'is_vendor' || values.accountType === 'is_customer') {
...
for (const key in newValues) {
  if (newValues.hasOwnProperty(key)) {
    formData.append(key, newValues[key]);
        ...
        await axios.post(`${url}/api/auth/signup/vendor/`,formData)
        .then(()=>{
            toast.success('User successfully created', {
                id: 'success',
            })
            setLoading(false)
            router.push('/duka/login')
        })
        .catch((err)=>{
  ...
  return (
    <main className="p-5 h-screen flex flex-col">
        ...
        <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-[500px] mx-auto">
        ...
        <Link href='/duka/login' className="block">Already have an account? <span className="text-primary">Login</span></Link>
        </div>
      </form>
    </Form>
    </div>
